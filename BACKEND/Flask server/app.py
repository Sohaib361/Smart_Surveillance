...
# Configure your database connection (SQLAlchemy)
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:postgres@localhost/postgres'
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
db = SQLAlchemy(app)


# Define your UserCredentials and UserInfo models using SQLAlchemy (as shown in the previous code examples)
...
# Define your routes (signup and login endpoints)
...
def login():
    ...
    cur = conn.cursor()
    try:
        # Retrieve the CNIC for the given username and hashed password
        cur.execute("SELECT CNIC FROM user_credentials WHERE username = %s AND password_hash = %s",
                    (username, hashed_password))
        cnic = cur.fetchone()

        if cnic:
            # Retrieve user info and corresponding vehicle info using a JOIN
            cur.execute("""
            ...
            if user_info:
                # Extract the user and vehicle information
                user_info_dict = {
                    'cnic': user_info[0],
                    'username': username,
